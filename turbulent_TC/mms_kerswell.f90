...
!   CALL EQN_V%SET_BCS( AXIS=3 , BCS_MINUS=NEUMANN , BCS_PLUS=NEUMANN )
...
!   CALL EQN_W%SET_BCS( AXIS=3 , BCS_MINUS=NEUMANN , BCS_PLUS=NEUMANN )
...
subroutine dealiazing(u,v,w)
  ...
  subroutine update_dt(dt_inout,cfl_current,cfl_target)
    real(kind=8), intent(inout)        :: dt_inout
    real(kind=8), intent(in)           :: cfl_current
    real(kind=8), intent(in)           :: cfl_target
    real(kind=8),parameter             :: dt_min = 1E-12
    real(kind=8),parameter             :: dt_max = 0.5
    
    ! Controller parameters                                                                                                                                                                                                                   
    real(kind=8), parameter :: eps_cfl    = 1.0e-14_dp  ! below this CFL is considered zero                                                                                                                                                       
    real(kind=8), parameter :: grow_max   = 1.1_dp       ! max 10 % increase per step                                                                                                                                                             
    real(kind=8), parameter :: shrink_max = 0.5_dp       ! max 50 % decrease per step                                                                                                                                                             
    real(kind=8), parameter :: deadband   = 0.05_dp      ! no action if |ratio-1| < 5 %                                                                                                                                                           
    real(kind=8), parameter :: alpha      = 0.5_dp       ! damping exponent                                                                                                                                                                       

    real(kind=8) :: ratio, fac, dt_new
  ...
  subroutine init()
    ...
    integer, parameter :: N_MODES = 10
    real(DP), parameter :: NOISE_AMP = 1E-4
    real(DP) :: SPEC_AMP, NORM
    ...
    ! --- 1. Remplir en espace physique avec bruit aléatoire ---
    CALL RANDOM_SEED()
    CALL Random_Number(NOISE_U(IS(1):IE(1), IS(2):IE(2), IS(3):IE(3)))
    CALL Random_Number(NOISE_V(IS(1):IE(1), IS(2):IE(2), IS(3):IE(3)))
    CALL Random_Number(NOISE_W(IS(1):IE(1), IS(2):IE(2), IS(3):IE(3)))
    ...
    ! Centrer autour de 0
    NOISE_U = 2._DP*NOISE_U - 1._DP
    NOISE_V = 2._DP*NOISE_V - 1._DP
    NOISE_W = 2._DP*NOISE_W - 1._DP
    
    DGU = NOISE_U * NOISE_AMP
    DGV = NOISE_V * NOISE_AMP
    DGW = NOISE_W * NOISE_AMP
    
    ! --- 2. Passer en espace de Fourier ---
    call c2c_1m_x(DGU, plan_fwd_x)
    call c2c_1m_x(DGV, plan_fwd_x)
    call c2c_1m_x(DGW, plan_fwd_x)
    ...
    ! --- 3. Appliquer le spectre décroissant sur les N_MODES premiers modes ---
    ! Spectre E(k) ~ exp(-k) ou puissance : ici on choisit E(k) ~ 1/(k+1)^2
    ! et on met à zéro tout ce qui est au-delà de N_MODES
    
    do i = 1, N(1)
    ! Numéro de mode en x (convention FFT : modes 1..N/2, N/2+1..N)
       if (i <= N_MODES) then
          SPEC_AMP = exp(-5./3. * real(i-1, DP) / real(N_MODES, DP))  ! décroissant
       else
          SPEC_AMP = 0._DP  ! tronqué au-delà de N_MODES
       end if
       DGU(i, :, :) = U(i, :, :) * SPEC_AMP
       DGV(i, :, :) = V(i, :, :) * SPEC_AMP
       DGW(i, :, :) = W(i, :, :) * SPEC_AMP
    ...
    do i = 1, N(2)
       if (i <= N_MODES) then
          SPEC_AMP = exp(-5./3. * real(i-1, DP) / real(N_MODES, DP))
       else
          SPEC_AMP = 0._DP
       end if
       DGU_Y(:, i, :) = DGU_Y(:, i, :) * SPEC_AMP
       DGV_Y(:, i, :) = DGV_Y(:, i, :) * SPEC_AMP
       DGW_Y(:, i, :) = DGW_Y(:, i, :) * SPEC_AMP
    end do
    ...
    U = SIN(pi*Y)*SIN(pi*Z)*SIN(pi*X)*NOISE_U
    
     ! --- 4. Normaliser pour que l'intégrale du spectre vaille NOISE_AMP^2 ---
