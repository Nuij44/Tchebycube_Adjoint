program tcheby_1d
  ...
  real(kind=8),allocatable ,dimension(:,:,:) ::  UA_TOT
  ...
  INTEGER :: nb_iter
  namelist /parameters_physical/RE,prm_k
  ...
  CALL MPI_BCAST( prm_K, 1, MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, IERR ) 
  ...
  FORALL(I=ph%XST(1):ph%XEN(1),J=ph%XST(2):ph%XEN(2),K=ph%XST(3):ph%XEN(3))
     UA_TOT(I,J,K) = UA(I,J,K) + PRM_K/SQRT(R(I,J,K))
  END FORALL
  
  CALL COMPUTE_NON_LINEAR_TERMS(&
       A, Z, R, OPA, OPZ, OPR, UA_TOT, UZ, UR , NLAM1, NLZM1, NLRM1,&
  ...
  DO IT_time=1,nb_iter
     ...
     CALL COMPUTE_NON_LINEAR_TERMS(&
          A, Z, R, OPA, OPZ, OPR, UA_TOT, UZ, UR , NLA, NLZ, NLR,&
          DG01, DG02, DG03, DG04, DG05, DG06, DG07, DG08, DG09)
...
!             DY = (MSH_Y%X(is(2)+1)-MSH_Y%X(is(2)))
  ...
  subroutine preproc()
    ...
    call alloc_x(UA_TOT , OPT_GLOBAL=.TRUE.) ; UA_TOT = 0._DP
...
!    F_R = 0._DP
  ...
  real(kind=8) pure function udf_ua_kep(t,x,y,z)
    real(kind=8),intent(in)::t,x,y,z
    udf_ua_kep = prm_K/sqrt(z)
  end function udf_ua_kep
