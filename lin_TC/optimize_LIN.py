...
def fun_tchebycube(vector, **kwargs):
    ...
    command_base="./"+options.exe_dir+"/DAL_lin.x --input "+input_dir+" --output-dir data_fun --init-data data_fun/init.h5 > resu_fun.txt"

    command="srun --ntasks-per-node="+str(nprocs)+" --cpu-bind=threads -- "+command_base                                                                
    print(command)   
...
def jac_tchebycube(vector, **kwargs):
    ...
    command_base="./"+options.exe_dir+"/DAL_lin.x --input "+input_dir+" --output-dir data_fun --init-data data_fun/init.h5 --do-adjoint > resu_jac_dns.txt"
