...
def read_grad_strat(str_file,type='float64'):
    ...
    try:
        ...
        data_read = [np.reshape(grad_u1, (N[0],N[1],N[2])),np.reshape(grad_u2, (N[0],N[1],N[2])),np.reshape(grad_u3, (N[0],N[1],N[2])),np.reshape(grad_t, (N[0],N[1],N[2]))]
    except: 
...
def fun_tchebycube(vector, **kwargs):
    ...
    write_init_strat_h5(vector[0],vector[1],vector[2],vector[3],'data_fun/init.h5')
 
    command_base=" --bind-to core --map-by slot:PE=1 "+options.exe_dir+"/SSP_VWI.x --input "+input_dir+" --output-dir data_fun --init-data data_fun/init.h5 > resu_fun.txt"
...
def jac_tchebycube(vector, **kwargs):
    ...
    command_base=" --bind-to core --map-by slot:PE=1 "+options.exe_dir+"/SSP_VWI.x --input "+input_dir+" --output-dir data_fun --init-data data_fun/init.h5 --do-adjoint > resu_jac_dns.txt"
...
if __name__ == '__main__':
    ...
    N = [129, 33, 65]
    init_u = 2.*np.random.random(N)-np.ones(N)
    init_v = 2.*np.random.random(N)-np.ones(N)
    init_w = 2.*np.random.random(N)-np.ones(N)
    init_t = 2.*np.random.random(N)-np.ones(N)
    
    data_dns_start=[init_u, init_v, init_w,init_t]
    ...
    write_init_strat_h5(data_opt.X_opt[0],data_opt.X_opt[1],data_opt.X_opt[2],data_opt.X_opt[3],options.output+"/init_opt.h5")
