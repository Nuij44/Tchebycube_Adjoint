...
!     if (do_adj) then
     ...
     !Calcul de J(u) = int_domaine <\nabla u, \nabla u>

     CALL GRAD(A, Z, R, OPA, OPZ, OPR, UA, DG01, DG02, DG03)
     CALL GRAD(A, Z, R, OPA, OPZ, OPR, UZ, DG04, DG05, DG06)
     CALL GRAD(A, Z, R, OPA, OPZ, OPR, UR, DG07, DG08, DG09)
     DG10 = R * (DG01**2 + DG02**2 + DG03**2 + DG04**2 + DG05**2 + DG06**2 + DG07**2 + DG08**2 + DG09**2)

     CALL get_quadrature_hhi(quad,DG10,DG01,ph%xst,ph%xen,n(1),ph%yst,ph%yen,n(2),ph%zst,ph%zen,n(3))
     
     J_U = DG01(PH%XST(1),PH%XST(2),PH%XST(3))
...
!           DG06 = SAVE_UR(nb_iter-it_time+1,:,:,:)
        ...
        CALL LAPLACIAN(A,Z,R,OPA,OPZ,OPR,DG04,DG05,DG06,DG07,DG08,DG09,DG10,DG11,DG12)
        
        SA = (2._DP*UA-0.5_DP*UAM1)/DT - DG01 + 2._DP*DG07
        SZ = (2._DP*UZ-0.5_DP*UZM1)/DT - DG02 + 2._DP*DG08
        SR = (2._DP*UR-0.5_DP*URM1)/DT - DG03 + 2._DP*DG09
