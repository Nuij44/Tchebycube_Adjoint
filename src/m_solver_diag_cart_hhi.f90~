module m_solver_diag_cart_hhi
  use M_FOURIER_TRANSFORM
    
  implicit none
  ! nu
  type t_solver_diag_cart_hhi
   ...
   contains
     ...
     PROCEDURE :: SET_BCS => SOLVER_DIAG_CART_HHI_SET_BCS
     PROCEDURE :: SET_BVS => SOLVER_DIAG_CART_HHI_SET_BVS
     PROCEDURE :: SET_UBVS => SOLVER_DIAG_CART_HHI_SET_UNSTEADY_BVS
     PROCEDURE :: SET_PARAMS => SOLVER_DIAG_CART_HHI_SET_PARAMS

     
  end type t_solver_diag_cart_hhi
...
contains
  ...
  subroutine ifce_initialise(this,grid,opx,opy,opz,ph)
    ...
    use decomp_2d
    implicit none
    class(t_solver_diag_cart_hhi):: this
  ...
  subroutine set_bval(this,grid,udf_bc_left,udf_bc_right,ph)
    ...
    call solver_diag_hhi_set_boundary_field(&
         ...
         this%bc_val_l_z, this%bc_val_r_z, ph%xst, ph%xen )
  ...
  subroutine ifce_solve(this,fi,sfi,sigma,ph)
    ...
    call solver_diag_cart_hhi_solve(fi,sfi,sigma,&
  ...
  subroutine ifce_solve_poisson(this,fi,sfi,nu,ph)
    ...
    call solver_diag_cart_hhi_solve_poisson(fi,sfi,&
  ...
  SUBROUTINE SOLVER_DIAG_CART_HHI_SET_BVS(THIS,MESH,AXIS,UDF_MINUS,UDF_PLUS)
    use decomp_2d
    use m_mesh_base
    use mpi
    IMPLICIT NONE
    ...
    if (axis==3) then
       ...
       CALL SOLVER_DIAG_HHI_SET_BOUNDARY_FIELD(&
            ...
            UDF_MINUS,UDF_PLUS,THIS%BC_VAL_L_Z,THIS%BC_VAL_R_Z,PH%XST,PH%XEN)
    else
       
       call MPI_Abort(mpi_comm_world,0,ierr)
    end if
  end subroutine SOLVER_DIAG_CART_HHI_SET_BVS

  SUBROUTINE SOLVER_DIAG_CART_HHI_SET_UNSTEADY_BVS(THIS,TC,MESH,AXIS,UDF_MINUS,UDF_PLUS)
    use decomp_2d
    use m_mesh_base
    use mpi
    IMPLICIT NONE
  ...
  SUBROUTINE SOLVER_DIAG_CART_HHI_SET_BCS(THIS,AXIS,BCS_MINUS,BCS_PLUS)
    use decomp_2d
    use mpi
    IMPLICIT NONE
    CLASS(T_SOLVER_DIAG_CART_HHI) :: THIS
  ...
  end subroutine SOLVER_DIAG_CART_HHI_SET_BCS

  
  SUBROUTINE SOLVER_DIAG_CART_HHI_SET_PARAMS(THIS,NU,SIGMA )
    IMPLICIT NONE
    CLASS(T_SOLVER_DIAG_CART_HHI) :: THIS
    REAL(KIND=8) :: NU(3)
    REAL(KIND=8) :: SIGMA
  ...
  END SUBROUTINE SOLVER_DIAG_CART_HHI_SET_PARAMS
...
!!$    end if
  ...
  subroutine solver_diag_hhi_set_boundary_field(x,y,z,udf_bc_left,udf_bc_right,bvzl,bvzr,xst,xen)
    use m_numerics
    implicit none
    integer :: xst(3),xen(3)
  ...
  subroutine solver_diag_cart_hhi_solve(fi,sfi,sigma,&
       lbdx, nx, xst, xen, &
       lbdy, ny, yst, yen, &
  ...
  end subroutine solver_diag_cart_hhi_solve



  subroutine solver_diag_cart_hhi_solve_poisson(fi,sfi,&
       nux,lbdx, nx, xst, xen, &
       nuy,lbdy, ny, yst, yen, &
...
!!$    end forall
  ...
  end subroutine solver_diag_cart_hhi_solve_poisson
...
end module m_solver_diag_cart_hhi
