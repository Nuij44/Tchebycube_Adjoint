module m_solver_diag_cart_hhi_cplx
  use M_FOURIER_TRANSFORM
    
  implicit none
  ! nu
  type t_solver_diag_cart_hhi_cplx
     ...
     REAL(KIND=8) :: nu(3), sigma, omega ! diffusion coefficient
     
     REAL(KIND=8) :: alf_l_z,bet_l_z 
     REAL(KIND=8) :: alf_r_z,bet_r_z 
     
     COMPLEX(KIND=8), dimension(:,:)  , allocatable :: bc_val_l_z !
     COMPLEX(KIND=8), dimension(:,:)  , allocatable :: bc_val_r_z !
   ...
   contains
     ...
     PROCEDURE :: SET_BCS => SOLVER_DIAG_CART_HHI_CPLX_SET_BCS
     PROCEDURE :: SET_BVS => SOLVER_DIAG_CART_HHI_CPLX_SET_BVS
     PROCEDURE :: SET_UBVS => SOLVER_DIAG_CART_HHI_SET_UNSTEADY_BVS
     PROCEDURE :: SET_PARAMS => SOLVER_DIAG_CART_HHI_CPLX_SET_PARAMS

     
  end type t_solver_diag_cart_hhi_cplx
  
  COMPLEX(KIND=8), dimension(:,:,:), allocatable :: dg_cl_z
  REAL(KIND=8), dimension(:,:,:), allocatable :: LBD
  complex(KIND=8), dimension(:,:,:), allocatable :: LBD_Z
contains
  ...
  subroutine ifce_initialise(this,grid,opx,opy,opz,ph)
    ...
    use decomp_2d
    implicit none
    class(t_solver_diag_cart_hhi_cplx):: this
    ...
    CALL SOLVER_DIAG_HHI_CPLX_BUILD (THIS, &
  ...
  SUBROUTINE SOLVER_DIAG_CART_HHI_SET_UNSTEADY_BVS(THIS,TC,MESH,AXIS,UDF_U_MINUS,UDF_U_PLUS,UDF_V_MINUS,UDF_V_PLUS)
    use decomp_2d
    use m_mesh_base
    use mpi
    IMPLICIT NONE
    ...
    INTEGER                       :: AXIS
    PROCEDURE(UDF_TIMESPACE)      :: UDF_U_PLUS,UDF_V_PLUS
    PROCEDURE(UDF_TIMESPACE)      :: UDF_U_MINUS,UDF_V_MINUS
    ...
    if (axis==3) then
       ...
       CALL SOLVER_DIAG_HHI_SET_UNSTEADY_BOUNDARY_FIELD(&
            ...
            UDF_U_MINUS,UDF_U_PLUS,UDF_V_MINUS,UDF_V_PLUS,THIS%BC_VAL_L_Z,THIS%BC_VAL_R_Z,&
            PH%XST,PH%XEN,PH%YST,PH%YEN,PH%ZST,PH%ZEN )
    else
       
       call MPI_Abort(mpi_comm_world,0,ierr)
    end if
  end subroutine SOLVER_DIAG_CART_HHI_SET_UNSTEADY_BVS

  subroutine solver_diag_hhi_set_unsteady_boundary_field(x,y,z,tc,udf_u_bc_left,udf_u_bc_right,udf_v_bc_left,udf_v_bc_right,bvzl,bvzr,xst,xen,yst,yen,zst,zen)
    use m_numerics
    implicit none
    integer :: xst(3),xen(3)
    integer :: yst(3),yen(3)
    ...
    complex(kind=8), dimension(xst(1):xen(1),xst(2):xen(2)) :: bvzl
    complex(kind=8), dimension(xst(1):xen(1),xst(2):xen(2)) :: bvzr
    PROCEDURE(UDF_TIMESPACE)      :: UDF_U_BC_LEFT,UDF_V_BC_LEFT
    PROCEDURE(UDF_TIMESPACE)      :: UDF_U_BC_RIGHT,UDF_V_BC_RIGHT
    integer :: i,j
    ...
    forall(i=xst(1):xen(1),j=xst(2):xen(2))
       bvzl(i,j) = cmplx(udf_u_bc_left(tc,X(I),Y(J),ZMIN),udf_v_bc_left(tc,X(I),Y(J),ZMIN))
    end forall

    
    forall(i=xst(1):xen(1),j=xst(2):xen(2))
       bvzr(i,j) = cmplx(udf_u_bc_right(tc,X(I),Y(J),ZMAX),udf_v_bc_right(tc,X(I),Y(J),ZMAX))
    end forall
    
  end subroutine solver_diag_hhi_set_unsteady_boundary_field


  subroutine ifce_solve(this,u,v,su,sv,sigma,omega,ph)
    use decomp_2d
    implicit none
    class(t_solver_diag_cart_hhi_cplx) :: this
    real(kind=8),dimension(:,:,:),allocatable :: u,v,su,sv
    real(kind=8) :: sigma,omega
    integer :: n(3)
    type(decomp_info) :: ph


    call solver_diag_cart_hhi_cplx_solve(u,v,su,sv,sigma,omega,&
  ...
  SUBROUTINE SOLVER_DIAG_CART_HHI_CPLX_SET_BVS(THIS,MESH,AXIS,UDF_U_MINUS,UDF_U_PLUS,UDF_V_MINUS,UDF_V_PLUS)
    use decomp_2d
    use m_mesh_base
    use mpi
    IMPLICIT NONE
    ...
    if (axis==3) then
       ...
       CALL SOLVER_DIAG_HHI_CPLX_SET_BOUNDARY_FIELD(&
            MESH(1)%X, MESH(2)%X, MESH(3)%X, &
            UDF_U_MINUS,UDF_U_PLUS,UDF_V_MINUS,UDF_V_PLUS,THIS%BC_VAL_L_Z,THIS%BC_VAL_R_Z,PH%XST,PH%XEN)
    else
       
       call MPI_Abort(mpi_comm_world,0,ierr)
    end if
  end subroutine SOLVER_DIAG_CART_HHI_CPLX_SET_BVS



  SUBROUTINE SOLVER_DIAG_CART_HHI_CPLX_SET_BCS(THIS,AXIS,BCS_MINUS,BCS_PLUS)
    use decomp_2d
    use mpi
    IMPLICIT NONE
    CLASS(T_SOLVER_DIAG_CART_HHI_CPLX) :: THIS
  ...
  end subroutine SOLVER_DIAG_CART_HHI_CPLX_SET_BCS

  
  SUBROUTINE SOLVER_DIAG_CART_HHI_CPLX_SET_PARAMS(THIS,NU,SIGMA,omega )
    IMPLICIT NONE
    CLASS(T_SOLVER_DIAG_CART_HHI_CPLX) :: THIS
    REAL(KIND=8) :: NU(3)
    REAL(KIND=8) :: SIGMA,OMEGA
  ...
  END SUBROUTINE SOLVER_DIAG_CART_HHI_CPLX_SET_PARAMS
  ...
  subroutine reduction_operator(D2,BL,BR,P,PM1,LBD,CL,CC,FL,FR,N)
    ...
    subroutine solver_diag_hhi_cplx_build(this,&
         x, nux, nx, xst, xen, &
         y, nuy, ny, yst, yen, &
    ...
    allocate( pz  ( 1:nz+1 , 1:nz+1 ) ) ; pz = 0
    allocate( pzm1( 1:nz+1 , 1:nz+1 ) ) ; pzm1 = 0
    allocate( lbdz( 1:nz+1          ) ) ; lbdz = 0 

    allocate( flz ( 1:nz+1         ) ) ; flz = 0
    allocate( frz ( 1:nz+1         ) ) ; frz = 0
...
!!$    if (nrank==0) then
!!$       print*,this%n(1),this%n(2),this%n(3)
!!$       print*,this%nu(1),this%nu(2),this%nu(3)
!!$    end if
    ...
    call reduction_operator( &
         d2, bl, br, pz(1:nzp,1:nzp), pzm1(1:nzp,1:nzp), lbdz(1:nzp), & 
         clz, ccz, flz(1:nzp),frz(1:nzp), nz )
    
    allocate( this%pz  ( 1:nz+1 , 1:nz+1 ) ) ; this%pz = pz
    allocate( this%pzm1( 1:nz+1 , 1:nz+1 ) ) ; this%pzm1 = pzm1
    allocate( this%lbdz( 1:nz+1          ) ) ; this%lbdz = lbdz 

    allocate( this%flz ( 1:nz+1          ) ) ; this%flz = flz
    allocate( this%frz ( 1:nz+1          ) ) ; this%frz = frz
    ...
    if (.not.allocated(dg_cl_z)) then
       ...
       allocate( LBD_Z(zst(1):zen(1),zst(2):zen(2),zst(3):zen(3) ) )
       LBD = 0
       LBD_Z = 0
    end if
...
!!$    end if
  ...
  end subroutine solver_diag_hhi_cplx_build

  subroutine solver_diag_hhi_cplx_set_boundary_field(x,y,z,udf_u_bc_left,udf_u_bc_right,udf_v_bc_left,udf_v_bc_right,bvzl,bvzr,xst,xen)
    use m_numerics
    implicit none
    integer :: xst(3),xen(3)
    real(kind=8), dimension(:),allocatable :: x,y,z
    ...
    forall(i=xst(1):xen(1),j=xst(2):xen(2))
       bvzl(i,j) = cmplx(udf_u_bc_left(0d0,X(I),Y(J),ZMIN),udf_v_bc_left(0d0,X(I),Y(J),ZMIN))
    end forall

    
    forall(i=xst(1):xen(1),j=xst(2):xen(2))
       bvzr(i,j) = cmplx(udf_u_bc_right(0d0,X(I),Y(J),ZMAX),udf_v_bc_right(0d0,X(I),Y(J),ZMAX))
    end forall
    
    
  end subroutine solver_diag_hhi_cplx_set_boundary_field
  
  subroutine solver_diag_cart_hhi_cplx_solve(u,v,su,sv,sigma,omega,&
       lbdx, nx, xst, xen, &
       lbdy, ny, yst, yen, &
    ...
    integer :: ny,yst(3),yen(3)
    integer :: nz,zst(3),zen(3)
    real(kind=8), dimension(xst(1):xen(1),xst(2):xen(2),xst(3):xen(3) ) :: u,su
    real(kind=8), dimension(xst(1):xen(1),xst(2):xen(2),xst(3):xen(3) ) :: v,sv
    ...
    forall(i=xst(1):xen(1),j=xst(2):xen(2),k=xst(3):xen(3))
       dg_x(i,j,k) = cmplx( su(i,j,k) , sv(i,j,k) )  - (fzl(k)*bvzl(i,j) + fzr(k)*bvzr(i,j))
    end forall
    
    DO K = ZST(3),ZEN(3)
       DO J = ZST(2),ZEN(2) 
          DO I=ZST(1),ZEN(1)
             LBD_Z(I,J,K) = ( lbdx(i) + lbdy(j) +  lbdz(k) + cmplx(sigma,omega))**(-1)
             IF (K==   1) LBD_Z(I,J,K) = 1
             IF (K==NZ+1) LBD_Z(I,J,K) = 1
          END DO
       END DO
    END DO
    ...
    forall(i=zst(1):zen(1),j=zst(2):zen(2),k=zst(3):zen(3))
       zg1_z(i,j,k) = zg2_z(i,j,k) * LBD_Z(i,j,k)
    end forall
    ...
    forall(i=xst(1):xen(1),j=xst(2):xen(2),k=xst(3):xen(3))
       u(i,j,k) = real(dg_x(i,j,k))
       v(i,j,k) =aimag(dg_x(i,j,k))
    end forall
  end subroutine solver_diag_cart_hhi_cplx_solve
...
end module m_solver_diag_cart_hhi_cplx
