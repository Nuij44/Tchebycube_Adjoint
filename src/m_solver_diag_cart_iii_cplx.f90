module m_solver_diag_cart_iii_cplx
  ...
  type t_solver_diag_cart_iii_cplx
     ...
     COMPLEX(KIND=8), dimension(:,:)  , allocatable :: bc_val_l_x, bc_val_r_x
     COMPLEX(KIND=8), dimension(:,:)  , allocatable :: bc_val_l_y, bc_val_r_y
     COMPLEX(KIND=8), dimension(:,:)  , allocatable :: bc_val_l_z, bc_val_r_z
   ...
   contains
     ...
     PROCEDURE :: SET_BCS => SOLVER_DIAG_CART_III_CPLX_SET_BCS
     PROCEDURE :: SET_BVS => SOLVER_DIAG_CART_III_CPLX_SET_BVS
     PROCEDURE :: SET_PARAMS => SOLVER_DIAG_CART_III_CPLX_SET_PARAMS
     
  end type t_solver_diag_cart_iii_cplx
...
contains
  ...
  subroutine ifce_initialise(this,grid,opx,opy,opz,ph)
    ...
    use decomp_2d
    implicit none
    class(t_solver_diag_cart_iii_cplx):: this
  ...
  SUBROUTINE SOLVER_DIAG_CART_III_CPLX_SET_BCS(THIS,AXIS,BCS_MINUS,BCS_PLUS)
    use decomp_2d
    use mpi
    IMPLICIT NONE
    CLASS(T_SOLVER_DIAG_CART_III_CPLX) :: THIS
  ...
  end subroutine SOLVER_DIAG_CART_III_CPLX_SET_BCS

  SUBROUTINE SOLVER_DIAG_CART_III_CPLX_SET_PARAMS(THIS,NU,SIGMA,OMEGA )
    IMPLICIT NONE
    CLASS(T_SOLVER_DIAG_CART_III_CPLX) :: THIS
    REAL(KIND=8) :: NU(3)
  ...
  END SUBROUTINE SOLVER_DIAG_CART_III_CPLX_SET_PARAMS
  ...
  subroutine ifce_solve(this,u,v,su,sv,sigma,omega,ph)
    ...
    implicit none
    class(t_solver_diag_cart_iii_cplx) :: this
    real(kind=8),dimension(:,:,:),allocatable :: u,su
    real(kind=8),dimension(:,:,:),allocatable :: v,sv
    real(kind=8) :: sigma,omega
    integer :: n(3)
    type(decomp_info) :: ph
    
    call solver_diag_cart_iii_cplx_solve(u,v,su,sv,sigma,omega,&
         ! xxxx
  ...
  subroutine reduction_operator(D2,BL,BR,P,PM1,LBD,CL,CC,FL,FR,N)
    ...
    allocate( dg_cl_x(1:2,xst(2):xen(2),yst(3):yen(3)))
  ...
  SUBROUTINE SOLVER_DIAG_CART_III_SET_PARAMS(THIS,NU,SIGMA,OMEGA )
    IMPLICIT NONE
    CLASS(T_SOLVER_DIAG_CART_III_CPLX) :: THIS
    REAL(KIND=8) :: NU(3)
    REAL(KIND=8) :: SIGMA
  ...
  SUBROUTINE SOLVER_DIAG_CART_III_CPLX_SET_BVS(THIS,MESH,AXIS,UDF_U_MINUS,UDF_U_PLUS,UDF_V_MINUS,UDF_V_PLUS)
    use decomp_2d
    use m_mesh_base
    use mpi
    IMPLICIT NONE
    ...
    select case(axis)
       case(1)
          CALL SOLVER_DIAG_III_CPLX_SET_BOUNDARY_FIELD_X(&
               MESH(1)%X, MESH(2)%X, MESH(3)%X, &
               UDF_U_MINUS,UDF_U_PLUS,UDF_V_MINUS,UDF_V_PLUS,THIS%BC_VAL_L_X,THIS%BC_VAL_R_X,PH%XST,PH%XEN,MESH%N)
       case(2)
          CALL SOLVER_DIAG_III_CPLX_SET_BOUNDARY_FIELD_Y(&
               MESH(1)%X, MESH(2)%X, MESH(3)%X, &
               UDF_U_MINUS,UDF_U_PLUS,UDF_V_MINUS,UDF_V_PLUS,THIS%BC_VAL_L_Y,THIS%BC_VAL_R_Y,PH%XST,PH%XEN,MESH%N)
       case(3)
          CALL SOLVER_DIAG_III_CPLX_SET_BOUNDARY_FIELD_Z(&
               MESH(1)%X, MESH(2)%X, MESH(3)%X, &
               UDF_U_MINUS,UDF_U_PLUS,UDF_V_MINUS,UDF_V_PLUS,THIS%BC_VAL_L_Z,THIS%BC_VAL_R_Z,PH%XST,PH%XEN,MESH%N)
       CASE DEFAULT
          call MPI_Abort(mpi_comm_world,0,ierr)
       end select


     end subroutine SOLVER_DIAG_CART_III_CPLX_SET_BVS

  
  subroutine solver_diag_iii_CPLX_set_boundary_field_x(x,y,z,&
       udf_u_bc_left, udf_u_bc_right, udf_v_bc_left, udf_v_bc_right, bvxl, bvxr ,xst, xen,n)
    use m_numerics
    implicit none
    integer :: xst(3),xen(3),n(3)
    real(kind=8), dimension(:),allocatable :: x,y,z
    COMPLEX(kind=8), dimension(xst(2):xen(2),xst(3):xen(3)) :: bvxl,bvxr
      PROCEDURE(UDF_TIMESPACE) :: UDF_U_BC_LEFT , UDF_U_BC_RIGHT , UDF_V_BC_LEFT , UDF_V_BC_RIGHT
    ...
    forall(j=xst(2):xen(2),k=xst(3):xen(3))
       bvxl(j,k) = CMPLX(udf_u_bc_left(0d0,XMIN,Y(J),Z(K)),udf_v_bc_left(0d0,XMIN,Y(J),Z(K)))
    end forall
    
    forall(j=xst(2):xen(2),k=xst(3):xen(3))
       bvxr(j,k) = CMPLX(udf_u_bc_right(0d0,XMAX,Y(J),Z(K)),udf_v_bc_right(0d0,XMAX,Y(J),Z(K)))
    end forall
    
  end subroutine solver_diag_iii_CPLX_set_boundary_field_x
  
  subroutine solver_diag_iii_CPLX_set_boundary_field_y(x,y,z,&
       udf_u_bc_left, udf_u_bc_right, udf_v_bc_left, udf_v_bc_right, bvyl, bvyr ,xst, xen,n)
    use m_numerics
    implicit none
    integer :: xst(3),xen(3),n(3)
    real(kind=8), dimension(:),allocatable :: x,y,z
    COMPLEX(kind=8), dimension(xst(1):xen(1),xst(3):xen(3)) :: bvyl,bvyr
    PROCEDURE(UDF_TIMESPACE) :: UDF_U_BC_LEFT , UDF_U_BC_RIGHT , UDF_V_BC_LEFT , UDF_V_BC_RIGHT
    ...
    forall(i=xst(1):xen(1),k=xst(3):xen(3))
       bvyl(i,k) = CMPLX(udf_u_bc_left(0d0,X(I),YMIN,Z(K)),udf_v_bc_left(0d0,X(I),YMIN,Z(K)))
    end forall
    
    forall(i=xst(1):xen(1),k=xst(3):xen(3))
       bvyr(i,k) = CMPLX(udf_u_bc_right(0d0,X(I),YMAX,Z(K)),udf_v_bc_right(0d0,X(I),YMAX,Z(K)))
    end forall
    
  end subroutine solver_diag_iii_CPLX_set_boundary_field_y
  
  subroutine solver_diag_iii_CPLX_set_boundary_field_z(x,y,z,&
       udf_u_bc_left, udf_u_bc_right, udf_v_bc_left, udf_v_bc_right, bvzl, bvzr ,xst, xen,n)
    use m_numerics
    implicit none
    integer :: xst(3),xen(3),n(3)
    real(kind=8), dimension(:),allocatable :: x,y,z
    COMPLEX(kind=8), dimension(xst(1):xen(1),xst(2):xen(2)) :: bvzl,bvzr
    PROCEDURE(UDF_TIMESPACE) :: UDF_U_BC_LEFT , UDF_U_BC_RIGHT , UDF_V_BC_LEFT , UDF_V_BC_RIGHT
  ...
  end subroutine solver_diag_iii_CPLX_set_boundary_field_z
  
  subroutine solver_diag_cart_iii_cplx_solve(u,v,su,sv,sigma,omega,&
    ...
    COMPLEX(kind=8), dimension(xst(2):xen(2),xst(3):xen(3)) :: bvxl
    COMPLEX(kind=8), dimension(xst(2):xen(2),xst(3):xen(3)) :: bvxr
    ...
    COMPLEX(kind=8), dimension(xst(1):xen(1),xst(3):xen(3)) :: bvyl
    COMPLEX(kind=8), dimension(xst(1):xen(1),xst(3):xen(3)) :: bvyr
...
!    DG_X = cmplx( SU,SV )
    ...
    DO K = ZST(3),ZEN(3)
       DO J = ZST(2),ZEN(2) 
          DO I=ZST(1),ZEN(1)
             !
             DG_LBD(I,J,K) = ( LBDX(I) + LBDY(J) +  LBDZ(K) + cmplx(SIGMA,omega) )**(-1)
    ...
    call tensor_product_i_cl(ccx, zg_x, dg_cl_x, xst, xen )
    IS = GET_IS_X([0,0,0])
    IE = GET_IE_X([0,0,0])    
    forall(j=is(2):ie(2),k=is(3):ie(3))
  ...
  end subroutine solver_diag_cart_iii_cplx_solve
...
end module m_solver_diag_cart_iii_cplx
