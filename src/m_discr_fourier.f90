module m_discr_fourier
  use m_mesh_base
  
  implicit none
  
  type t_discr_fourier
     integer :: N ! nb
   ...
   contains
     ...
     procedure :: init_discr_fourier , check_discr_fourier , free_discr_fourier
     procedure :: eval_discr_fourier_x,eval_discr_fourier_y,eval_discr_fourier_z

  end type t_discr_fourier

  public ::  init_discr_fourier , check_discr_fourier , free_discr_fourier
...
contains
  ...
  subroutine init_discr_fourier(this,msh,opt)
    use decomp_2d
    use decomp_2d_mpi   
    use m_mesh_base
    ...
    implicit none
    !.. args
    class(t_discr_fourier ),target :: this
    ...
    do i=0,n
       do j=0,n
          if (i==j) then
             ...
             d(i+1,j+1) = (0.5)*(-1)**(i+j)*sin((i-j)*pi/(n+1))**(-1)
          end if
       end do
    end do
  ...
  end subroutine init_discr_fourier
  
  subroutine check_discr_fourier(this)
    implicit none
    !.. 
    class(t_discr_fourier ) :: this
    !.. variables
    ...
    !.. Checking for first order derivation on Fouriershev polynomials
    do k=0,N
    ...
    !.. Checking second order derivation on Fouriershev polynomials
  ...
  end subroutine check_discr_fourier
  
  
  subroutine free_discr_fourier(this)
    implicit none
    class(t_discr_fourier ) :: this
    deallocate(this%A)
  end subroutine free_discr_fourier
  
  SUBROUTINE eval_discr_fourier_x(this,fi,dfi)
    use decomp_2d
    implicit none
    CLASS(t_discr_fourier) :: this
  ...
  END SUBROUTINE EVAL_DISCR_FOURIER_X


  SUBROUTINE eval_discr_fourier_y(this,fi,dfi,DG1Y,DG2Y)
use decomp_2d
   use decomp_2d_fft
  ...
  END SUBROUTINE EVAL_DISCR_FOURIER_Y

  SUBROUTINE eval_discr_fourier_z(this,fi,dfi,DG1Y,DG2Y,DG1Z,DG2Z)
    use decomp_2d
    implicit none
    CLASS(t_discr_fourier) :: this
    real(kind=8),dimension(:,:,:),allocatable :: fi,dfi
  ...
  END SUBROUTINE EVAL_DISCR_FOURIER_Z
