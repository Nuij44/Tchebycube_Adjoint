...
module m_solver_diag_cart_fft
  ...
  type , extends(t_solver_diag_base) :: t_solver_diag_cart_fft
   ...
   contains 
     ...
     procedure :: SET_BVS_TIME => SOLVER_DIAG_CART_FFT_SET_BVS_VALUES_UNSTEADY
  ...
  end type t_solver_diag_cart_fft
...
contains 
  SUBROUTINE SOLVER_DIAG_CART_FFT_SET_PARAMS(THIS,DIM,NU,sigma)
    IMPLICIT NONE
    CLASS(T_SOLVER_DIAG_CART_FFT) :: THIS
    integer                       :: dim(3)
    real(dp)                      :: nu
    real(dp)                      :: sigma
 ...
 !   par = sum(sfi**2)
 !   CALL MPI_ALLREDUCE(MPI_IN_PLACE,par,1,MPI_REAL8,MPI_SUM,MPI_COMM_WORLD,IERR)
!    if (nrank==0) print*,"dbg",par
  ...
  SUBROUTINE SOLVER_DIAG_CART_FFT_SET_BVS_VALUES_UNSTEADY(THIS,time,MSH,AXIS,UDF_MINUS,UDF_PLUS)
      use m_boundary_conditions
      use m_lap1d_tcheby
    ...
    end subroutine SOLVER_DIAG_CART_FFT_SET_BVS_VALUES_UNSTEADY
...
!        END DO
  ...
  end module m_solver_diag_cart_fft
