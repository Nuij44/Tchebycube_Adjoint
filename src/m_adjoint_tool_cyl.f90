module m_adjoint_tool_cyl
  ...
  INTERFACE SAVE_HDF5
    ...
    MODULE PROCEDURE EXPORT_HDF5_VELOCITY
  ...
  subroutine COMPUTE_STRAT_ADJOINT_NON_LINEAR_TERMS_CYL(A,Z,R,&
    op_A,op_Z,op_R, UA, UZ, UR, t_adj, nlt,dg01, dg02, dg03)
    
    implicit none
    
    class(t_operator_base)     :: op_A, op_Z, op_R
    REAL(KIND=8),DIMENSION(:,:,:),ALLOCATABLE :: UA,UR,UZ,T_ADJ,NLT,A,Z,R
    REAL(KIND=8),DIMENSION(:,:,:),ALLOCATABLE :: DG01,DG02,DG03

    CALL OP_A%D1(T_ADJ,DG01)
    CALL OP_Z%D1(T_ADJ,DG02)
    CALL OP_R%D1(T_ADJ,DG03)

    NLT = UA*DG01/R + UZ*DG02 + UR*DG03

  END SUBROUTINE COMPUTE_STRAT_ADJOINT_NON_LINEAR_TERMS_CYL


end module m_adjoint_tool_cyl
